...
- Pull a released model from `krispeterson/repath-model`:
  ...
  - For local ad-hoc exports, see `assets/models/README.md`.
...
4) Install project dependencies:
   ...
   npm install
   ```
5) Pull the currently configured model release:
   ```bash
   npm run pull:model:release
   ```

6) (Optional) Export a YOLOv8 TFLite model and labels locally if you need to test ad-hoc model changes.
   See `assets/models/README.md` for when and why this is needed.
...
7) Create a development build (required for VisionCamera + TFLite):
   ```bash
   npm run prebuild
   npm run android
   ```
...
- Model binaries in `assets/models/` are local files. Labels/config metadata are tracked.
  See `assets/models/README.md` for model export details and quantization options.

## Optional ML Workspace (Advanced)

Most developers can skip this and focus on app behavior.
Use this only when benchmark/dataset workflows are needed in mobile context.
Primary model training/release workflows now live in `repath-model`.
